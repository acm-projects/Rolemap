...
- "index": the result number you selected
- "description": 1-2 sentence description of what this resource covers and why it's useful (do NOT start with "Learn")
- "objectives": array of 2-3 short bullet points describing what the learner should take away from this resource

Example: [{{"index": 1, "description": "Official AWS guide...", "objectives": ["Understand...", "Configure..."]}}]
No markdown, no extra text, just the JSON array."""
        ...
        if response_text.startswith('[') and response_text.endswith(']'):
            ...
            for item in selected:
                idx = item if isinstance(item, int) else item.get("index", 0)
                if 1 <= idx <= len(search_results) and len(curated_results) < count:
                    result = search_results[idx - 1]
                    desc = item.get("description", "") if isinstance(item, dict) else ""
                    objectives = item.get("objectives", []) if isinstance(item, dict) else []
                    curated_results.append({
                        "title": result.get("title", "Resource")[:80],
                        "description": desc or f"Covers {subtopic} as part of {concept}.",
                        "url": result.get("url"),
                        "type": task_type,
...
def fallback_rule_curation(concept: str, subtopic: str, task_type: str, search_results: List[Dict], count: int) -> List[Dict]:
    ...
    for result in prioritized[:count]:
        if result.get("url"):
            ...
            # Generate generic but useful objectives from context
            objectives = [
                f"Understand the fundamentals of {subtopic}",
                f"Apply key {concept} concepts from this resource",
                f"Identify how {subtopic} fits into {concept} workflows",
            ]
            curated_tasks.append({
                "title": result.get("title", "Resource")[:80],
                "description": f"Learn {subtopic}: {snippet}",
                "url": result.get("url"),
                "type": task_type,
